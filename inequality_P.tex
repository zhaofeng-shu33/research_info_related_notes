...
C_1 u_i & = \log \frac{p_i}{q_i} + \lambda_2(p, q) \label{eq:deriv2}\\
...
follows $A=-\frac{e^{-\lambda_2}}{C_2}$. A final constraint is to make sure the equality
in \eqref{eq:uPQ} holds, and we can obtain from that $\lambda_1 + \lambda_2 = 1$. 
...
D_{\textrm{KL}}(P, Q) = \sup_{\phi: \mathcal{X} \to R} C_1 \mathbb{E}_P(\phi) -\lambda_2 +1 - \mathbb{E}_Q(e^{-\lambda_2 + C_1 \phi})
\end{equation}
