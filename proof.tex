...
We define $\mu=\frac{1}{n}\mathbbm{1}_n^T h$ and $\lambda = -u/n, \epsilon=2\lambda$.
Then we choose $\lambda_1=\mu, \lambda_{i+1}=g_i\lambda + \frac{\epsilon}{2}$
...
\begin{align}
\diag(y) + \Xi - \widetilde{B} & = \begin{pmatrix} h^T (g+\mathbbm{1}_n) & -h^T +(\mu + \frac{\epsilon}{2} )\mathbbm{1}_n^T + \lambda g^T\\
-h+(\mu + \frac{\epsilon}{2} )\mathbbm{1}_n + \lambda g& \Xi_n \end{pmatrix}
\succeq 0 \notag \\
...
By the construction $\tilde{g}$ is the eigenvector of $\diag(y) + \Xi - \tilde{B}$ with eigenvalue $0$.
...
\end{align*}
Using $\tilde{h}^T\mathbbm{1}_n=\mu$ and Law of Large Numbers (Chebyshev's inequality), with large probability (exponential rate), we can approximate
...
-\frac{\mathbb{E}^2[\tilde{h}_1]+\mathbb{E}^2[\tilde{h}_2]}{2} + \frac{\mu^2}{n^2}\\
&= \frac{\Var[\tilde{h}_1]+\Var[\tilde{h}_2]}{2}
= O(\log n)
\end{align*}		
