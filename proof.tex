...
\begin{proof}
	...
	$f(\sigma) = \sum_{i,j} A_{ij}\delta_{\sigma_j \neq \sigma_j}$ where $A_{ij}$ is the random adjacency matrix. We also assume the ground truth
	is $\tilde{\sigma} = (0,1,0,1,\dots,)$. Event $A_{12}$ is equivalent to say $\bar{\sigma}=(1,0,0,1,0,1,\dots,)$ is the minimal value of $f(\sigma)$.
	Then $P(A_{12}) = 1 - P(\exists \sigma, s.t. f(\sigma)>f(\bar{\sigma}))$.
	Further:
	\begin{align*}
	P(\exists \sigma, s.t. f(\sigma) < f(\bar{\sigma})) &=
	P(\exists \sigma, s.t. f(\sigma) < f(\bar{\sigma})| f(\bar{\sigma}) < f(\tilde{\sigma}))P(f(\bar{\sigma}) > f(\tilde{\sigma})) \\
	&+ P(\exists \sigma, s.t. f(\sigma) < f(\bar{\sigma}) | f(\bar{\sigma}) \geq f(\tilde{\sigma}))P(f(\bar{\sigma}) \leq f(\tilde{\sigma}))
	\end{align*}
	We have shown previously $P(f(\bar{\sigma}) \leq f(\tilde{\sigma})) = P(\sum_{i=1}^{n-2} Z_i - X_i \geq 0) = \Theta(n^{-(\sqrt{a} - \sqrt{b})^2})$
	where $Z_i \sim Bern(\frac{b\log n}{n})$ and $X_i \sim Bern(\frac{a \log n}{n})$.
	Therefore,
	$$
	P(\exists \sigma, s.t. f(\sigma) < f(\bar{\sigma})) \leq 1 - \Theta(n^{-(\sqrt{a} - \sqrt{b})^2}) + \Theta(n^{-(\sqrt{a} - \sqrt{b})^2})P(\exists \sigma, s.t. f(\sigma) < f(\bar{\sigma}) | f(\bar{\sigma}) \leq f(\tilde{\sigma}))
	$$
	Using union bound (this proof is wrong),
	\begin{align*}
	&P(\exists \sigma, s.t. f(\sigma) < f(\bar{\sigma}) | f(\bar{\sigma}) \leq f(\tilde{\sigma}))
	\leq \sum_{\sigma \neq \tilde{\sigma}}P(f(\sigma) < f(\tilde{\sigma}) | f(\bar{\sigma}) \leq f(\tilde{\sigma}) ) \\
...
\frac{n(D_1 + D_2)}{2} & -D_1 & -D_1& \dots, & -D_1 & D_2 & \dots & D_2 \\
-D_1 & D_1 + d & a & \dots & a & b & \dots & b \\
\vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-D_1 & a & \dots & a & D_1 + d & b & \dots & b \\
D_2 & b & \dots & b & D_2 + d & a & \dots & a \\
\vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
D_2 & b & \dots & b & a & \dots & a & D_2 + d 
\end{pmatrix}
\end{equation}
...
% possibly useful to estimate the eigenvalue.
