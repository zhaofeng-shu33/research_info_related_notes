...
Then the problem is $\max h^T y + \frac{1}{2}y^T B y$.
...
s.t.\, \diag(X) &= 1 \\
 X &\succeq 0 \\
 \sum_{j=1, j\neq i}^{n+1} (X_{ij} + X_{ji})& = 0
\end{align*}
The number of constraints is $2(n+1)$. Let $\tilde{g} = (1,g^T)^T$.
...
We define $\mu=\mathbbm{1}_n^T h$ and $\lambda = -u/n$.
Then we choose $\lambda_1=\mu, \lambda_{i+1}=g_i\lambda$
and $y_i$ are chosen such that
$(\diag\{y_1, \dots, y_n\} + \Xi - \widetilde{B})\tilde{g}=0$, from which we obtain
\begin{align}
y_1 &= h^T g - n\lambda \\
y_{i+1} & = (h_i - \mu - (n-2)\lambda)g_i - \lambda + \diag\{Bgg^T\}, i = 1, \dots, n
\end{align}
We can also verify that $\sum_{i=1}^{n+1} y_i = 2h^Tg + g^T \widetilde{B} g$.

 
Let $Y_i = \tilde{B}(\tilde{g}\tilde{g}^T)_{ii}$.
Then $Y = (h^T g, \diag\{ g^T+Bgg^T\})$
...
\succeq 0
$$
We first analyze $E[\diag(Y) - \tilde{B}]$, Let $D_1 = m\frac{D_{KL}(p_0||p_1)}{\log a/b},
D_2 = m\frac{D_{KL}(p_1||p_0)}{\log a/b}$. Then the constant matrix $\tilde{C}$ can be written as:
\begin{equation}
...
\end{pmatrix}
\end{equation}
where $a,b,d$ are defined:
\begin{align*}
a' & = - \frac{2a \log n}{n} + 1 \\
b' & = - \frac{2b \log n}{n} + 1 \\
d & = (a-b) \log n - \frac{2a \log n}{n} + 1
\end{align*}
which are similar to those in the proof of Lemma 11 in \cite{abbe}.
...
The first three eigenvalues can also be solved out by considering the $3\times 3 $
...
If all eigenvalue of $\diag(hg^T + Bgg^T) - B$ is larger than zero, then by
...
$h_i = \frac{1}{\log a/b}\sum_{j=1}^m \log \frac{p_0(x_i^{(j)})}{p_1(x_i^{(j)})} = \frac{1}{\log a/b}(D(X_i^{(m)} || p_1) - 
D(X_i^{(m)} || p_0)  ))
$, whose mean value is positive ($\frac{1}{\log a/b}D(p_0||p_1)$).
Therefore, we need to consider to control the following probability:
$P(h_i + \sum_{i=1}^{n/2} (x_i - z_i) \leq 0) \leq \exp(-\log n \min \theta^*)$ 
...
For $g_i = -1$, we have $\epsilon_i = g_i h_i = \frac{1}{\log a/b}(D(X_i^{(m)} || p_0) - 
...
$P(\diag(hg^T + Bgg^T) \textrm{ is not semidefnite}) \leq n^{1-(\sqrt{a} - \sqrt{b})/2 - \frac{1}{2}D_{1/2}(p_0||p_1)} = o(1)$
since $(\sqrt{a} - \sqrt{b})/2 + D_{1/2}(p_0||p_1) > 2$.

If we consider the eigenvalues of
$\diag(hg^T + Bgg^T) - B$ directly,
