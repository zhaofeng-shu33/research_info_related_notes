...
\begin{lemma}
	...
	\begin{align}
	\min \{D_{\frac{2}{3}}(p_0||p_1), 2D_{\frac{1}{3}}(p_0||p_1) \} & \geq \frac{3}{2} D_{1/2}(p_0||p_1) \notag\\
	(\sqrt{a}-\sqrt{b})^2-2 & > 3a^{1/3}b^{1/3}(a^{1/6}-b^{1/6})^2\label{eq:two_conditions}
	\end{align}
	are satisfied, then we can show that $P_e$ is lower bounded by
...
\begin{proof}
	...
	$$
	From the two conditions mentioned in \eqref{eq:two_conditions},
	we then have
	$$
	P(A) \leq n^{-\frac{3}{2} (\gamma \cdot D_{1/2}(p_0||p_1) + (\sqrt{a} - \sqrt{b})^2)}
	$$
