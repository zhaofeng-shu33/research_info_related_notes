...
\end{lemma}
\begin{proof}
	We follow the main idea of Abbe's proof in \cite{abbe} but we give a simper version.
	We want to compute $P(F_A)$ directly by considering
	$P(F_A) = 1 - P(F_A^c)$ where
	$F_A^c = \{B_i - A_i <0, \forall i \in A \}$.
	where $A_i:=|\{j \in A | (i,j) \in E \}|$ and $B_i := |\{ j \in B | (i,j) \in E \}|$.
	We notice that the events $\{B_i - A_i < 0\}$ and $\{B_j - A_j < 0\}$
	are not independent since there may be edges $(i, j) \in E$.
	We make the modifications and define new random variables $A'_j = A_j - \xi_{1j}$.
	The indicator function $\xi_{1j}$ is a random variable which takes value $1$
	if $(1,j) \in E(G)$, otherwise takes value $0$.
	Since $B_i - A_i <0$ implies $B_i - A'_i < 1$
	We then have
	\begin{align*}
	P(\{B_i - A_i <0, \forall i \in A \}) &= P(B_1 - A_1 < 0) P(\{B_i - A_i <0, \forall i \in A \} | B_1 - A_1 < 0)\\
	&\leq  P(B_1 - A_1 < 0) P(\{B_i - A'_i < 1, \forall i \in A \} | B_1 - A_1 < 0) \\
	&=  P(B_1 - A_1 < 0) P(\{B_i - A'_i < 1, \forall i \in A \}) \\
	\end{align*}
\end{proof}
