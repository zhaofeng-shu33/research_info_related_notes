...
\begin{proof}
	...
	\end{align*}
	We have shown previously $P(f(\bar{\sigma}) \geq f(\tilde{\sigma})) = P(\sum_{i=1}^{n-2} Z_i - X_i \geq 0) = \Theta(n^{-(\sqrt{a} - \sqrt{b})^2})$
	where $Z_i \sim Bern(\frac{b\log n}{n})$ and $X_i \sim Bern(\frac{a \log n}{n})$.
	Therefore,
	$$
	P(\exists \sigma, s.t. f(\sigma)>f(\bar{\sigma})) \leq 1 - \Theta(n^{-(\sqrt{a} - \sqrt{b})^2}) + \Theta(n^{-(\sqrt{a} - \sqrt{b})^2})P(\exists \sigma, s.t. f(\sigma)>f(\bar{\sigma}) | f(\bar{\sigma}) \geq f(\tilde{\sigma}))
	$$
	Using union bound,
	\begin{align*}
	&P(\exists \sigma, s.t. f(\sigma)>f(\bar{\sigma}) | f(\bar{\sigma}) \geq f(\tilde{\sigma}))
	\leq \sum_{\sigma \neq \tilde{\sigma}}P(f(\sigma)>f(\tilde{\sigma}) | f(\bar{\sigma}) \geq f(\tilde{\sigma}) ) \\
