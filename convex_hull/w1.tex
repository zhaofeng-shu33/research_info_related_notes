...
\end{equation}
Equation tells that for bivariate t distribution,
we have
\begin{align*}
    \E[V_N] & = 2\pi \int_{\R}
...
Let $y \sim U[0,1]$ and $\theta \sim
U[0, 2\pi]$. Then we obtain $r$
by $r=F^{-1}(y) = \sqrt{(1-y/C)^{-2/v}-1}$.
$F^{-1}$ is the inverse function
of $F(r)=C
\left(1-(r^2+1)^{-v/2}
\right)$, the CDF of
$p(r)= \frac{2\Gamma(v/2+1)}{v\Gamma(v/2)}
\frac{r}{(1+r^2/v)^{v/2 + 1}}$.
The constant $C=\frac{2}{v}\frac{\Gamma(v/2+1)}{
    \Gamma(v/2)
}$.
Finally $x,y$ are obtained from polar transformation
of $(r,\theta)$.
...
is of the order $(\log n)^{1-\frac{1}{2\beta}}$.
To prove this, we first show that when $
\beta \geq 1$, we can first obtain an upper bound
for $p(x, y)$ as $p(x,y)\leq C\exp(-\frac{1}{2} (x^{2\beta} 
+ y^{2\beta}))$.


The marginal distribution
$p(x) = 2C\exp(-x^{2\beta}/2) \leq C' \exp(-\frac{1}{2}x^{2\beta})$.
Since $1-\Phi(x) = \int_{x}^{\infty} p(u)du$,
Then we show that $1-\Phi(x) \leq C\frac{\exp(-x^{2\beta}/2)}{x^{2\beta - 1}}$
as $x\to \infty$.
By integration by part we have
\begin{align*}
    1-\Phi(x) &\leq C' \int_x^{+\infty} \exp(-\frac{1}{2}u^{2\beta})du \\
    &= \frac{C'}{\beta}
    \left(\frac{\exp(-x^{2\beta}/2)}{x^{2\beta-1}}
    -(2\beta-1) \int_{x}^{+\infty}
    \frac{\exp(-u^{2\beta}/2)}{u^{2\beta}}du
    \right)
\end{align*}
...
This proposition can be shown easily.
