...
\end{equation} 
Suppose $k\leq d-2$ and $k$ is odd,
then
\begin{align*}
    g(k,x,y,z)&=\int_u^z x
    (y^2-x^2)^{\frac{k-1}{2}}
    ...
    \\
    &= \frac{1}{2}\sum_{j=0}^{(k-1)/2}
    (-1)^{\frac{k-1}{2}-j}\binom{(k-1)/2}{j}
    B(\frac{d+k}{2}-j-1, \frac{d-k}{2})
    (y^2-u^2)^j (z^2-u^2)^{d-2-j}
\end{align*}
To prove \eqref{eq:ref_prove_integration},
we only need to show that
\begin{align*}
    \sum_{k\leq d-2}g(2\lceil(k+1)/2 \rceil -1,x,y,z)
    =2(y^2-u^2)^{\frac{d-2}{2}}
    (z^2-u^2)^{\frac{d-2}{2}}
\end{align*}
When $j=\frac{d-2}{2}$, we have.
\subsection{Polynomial tail}
