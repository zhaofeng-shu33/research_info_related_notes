...
\begin{proof}
    ...
    Let $p_1^2=1+p^2$
    Then we make the change of variables $t_i = p_1t'_i$.
...
For truncated tails, if $F(1-x) \sim x^k L(1/x)$,
then
\begin{align}
    K(1-x)
...
\begin{align}\label{eq:efn_truncated_formula}
    ...
    ^{\frac{d-1}{2k+d-1}} \\
    &\sim \frac{\tilde{b} }{d!}
    a^{\frac{d-1}{2k+d-1}}\Gamma\left(d+1-\frac{d-1}{2k+d-1}\right)
    N^{\frac{d-1}{2k+d-1}}
    L(N)
    ^{\frac{d-1}{2k+d-1}}
\end{align}
...
\end{equation}
When $d\to \infty$, using the 
following lemma, we obtain the asymptotic value of $\tilde{b}$ as
\begin{equation}
    \tilde{b} \sim 2\pi^{(d-1)/2}
    d^{-1}e^kd^{d/2}
\end{equation}
The asymptotic value of $a$ is given as
\begin{equation}
...
The relationship of $N$ with $k$ for fished $d$ is inverse to
those other two tails(polynomial and exponential tails).
The larger $k$ becomes, the smaller $N$ is. That is, faster decay to 0 at $x=1^{-1}$
