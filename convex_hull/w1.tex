...
we have:
\begin{align}\notag
    E[F_N] & \sim \binom{N}{d}\int_0^{+\infty} f(p) (1-D(v)p^{-v})^{N-d} dp
    = \frac{k_{d,v} v^{(vd+1)/2}}{d! v D_v^d} \\
    ...
    {(d-1)! \cdot \Gamma(\frac{v+1}{2})^d
    \Gamma(\frac{vd}{2} + 1)} =:g_d(v)\label{eq:F_N_general_d_t}
\end{align}
...
\begin{equation}
\textcolor{red}{    H(x) = \frac{1}{3}
    \int_x^{+\infty} (1-\frac{x}{z}) |dF^3(z)|}
\end{equation}
...
Suppose $F(x) = x^{-k} L(x)$, then from
\eqref{eq:G_from_F},
we can derive
\begin{align}
    G(x) & \sim \frac{L(x)x^{-k}}{2(k+1)} \\
    H(x) & \sim \frac{L^3(x)x^{-3k}}{3(3k+1)} 
    \sim \frac{8(k+1)^3}{3(3k+1)}
    G^3(x)
\end{align}
