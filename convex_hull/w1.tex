...
In Laplace's method, there is another case when $h'(0)=0$ happens.
This case can be used to obtain the result
of $\E[V_N]$ for uniform distribution.

\begin{theorem}
    Suppose $G(x) = t^{m} \phi(t)$, where $\phi(0)\neq 0$
    and $m$ is a non-negative integer.
    Further suppose $h'(x)<0$ for $x> 0$ and $h'(0)=0$.
    Then
