...
\begin{align}\notag
    ...
    (1-D(v)p^{-v})^{N-d} dp
    = \frac{k_{d,v} v^{(vd+1)/2}\Gamma(d)}{d! v D_v^d} \\
...
\begin{align}
    ...
    \left[
        \int_x^y (1-\frac{x}{z}) |dF(z)|
        + \int_y^{+\infty} (1-\frac{x}{y}) |dF(z)|
    \right]
    \\
    & = \int_x^{+\infty}(1-\frac{x}{z}) |d(F_1(z)F(z))|
\end{align}
where $\cos\alpha=\frac{x}{y}, \cos\beta = \frac{x}{z}$.
and $z^*=\frac{zy\sin \theta }{\sqrt{z^2+y^2-2zy\cos\theta}}$.
...
\begin{align}
    ...
    H(x) & \sim \frac{2k^{-3k/2-1}}{3k B(3k/2, 1/2)} x^{-3k}L(3x) ?
\end{align}
