...
\begin{frame}
    ...
    \begin{itemize}
        ...
        $\E[V_N] \sim D_{\alpha}$
        \item Exponential tails: $p(x,y) = C_{\alpha}(x^2+y^2)^{\alpha} \exp(-(x^2+y^2)^{\beta}/2)$
        ,$\E[V_N] \sim D (\log N)^{2-\frac{3}{2\beta}}$
    ...
    \end{itemize}
\end{frame}
\begin{frame}{Tails family}
    finite linear combination: the rate depends on the dominant term.
\end{frame}
\begin{frame}
